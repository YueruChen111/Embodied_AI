...
if len(target_objects) == 0:
    ...
    execute(target_objects[0].position, [], 5)
objects = ['person in red', 'person in blue', 'person in pink', 'person with umbrella', 'white car', 'green van',  'person in yellow'] 
# follow the person in blue
say('Ok - flying to the person in red')
while:
    target_objects = parse_query_obj('person in blue')
    if len(target_objects) == 0:
        say(f'Sorry - there is no person in blue')
        break 
    elif np.linalg.norm(x.position) <0.2:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            layout = [
                [sg.Text("please input your command: ")],
                [sg.Input(key="-INPUT-")],
            ...
            # 关闭窗口
            window.close()
            print("user's command: ", user_input)  #@param {allow-input: true, type:"string"}
            object_list = []
